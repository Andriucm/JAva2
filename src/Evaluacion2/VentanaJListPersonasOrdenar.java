...
public class VentanaJListPersonasOrdenar extends JFrame
	...
	private Persona p;
	private JRadioButton rdbtAscendente;
	private JRadioButton rdbtDescendente;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					VentanaJListPersonasOrdenar frame = new VentanaJListPersonasOrdenar();
	 ...
	 * Create the frame.
	 */
	public VentanaJListPersonasOrdenar() {
		setTitle("VentanaJListComplejos");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 1408, 466);
		...
		cmbOrdenar.setModel(new DefaultComboBoxModel<String>(new String[] { "DNI", "Nombre", "Apellidos", "Fecha" }));
		cmbOrdenar.setSelectedIndex(0);
		panel.add(cmbOrdenar);

		rdbtAscendente = new JRadioButton("Ascendente");
		rdbtAscendente.setSelected(true);
		panel.add(rdbtAscendente);

		rdbtDescendente = new JRadioButton("Descendente");
		panel.add(rdbtDescendente);
		bg = new ButtonGroup();
		bg.add(rdbtAscendente);
		bg.add(rdbtDescendente);
...
//		Compruebo si esta seleccionado Descendente
			if (rdbtDescendente.isSelected()) {
				orden = -1;
			}
			switch (seleccion) {
//		ordeno el arraylis de manera Personalizda por DNI
			case "DNI":
			...
			case "Apellidos":
				Collections.sort(listado, new Comparator<Persona>() {
					...
					public int compare(Persona p1, Persona p2) {
						...
						return (orden * p1.getApellido().compareTo(p2.getApellido()));
					}

				});
				break;
//			Ordeno el arraylist de manera Personalizada Por Fecha
			case "Fecha":
				Collections.sort(listado, new Comparator<Persona>() {
					@Override
					public int compare(Persona p1, Persona p2) {
	...
	public void focusGained(FocusEvent e) {
		...
		txtDia.select(0, txtDia.getText().length());
		txtAño.select(0, txtAño.getText().length());
